...
tape( 'main export is a function', function test( t ) {
	...
	t.strictEqual( typeof atleast2d, 'function', 'main export is a function' );
	t.end();
});

tape( 'the function converts a list of values (scalars and/or ndarrays) to ndarrays having at least two dimensions', function test( t ) {
	var out;
	var x;
	var y;
	var z;
	...
	out = atleast2d( [ x, y, z, w, u, v, s ] );

	t.strictEqual( isArray( out ), true, 'returns expected value' );
	t.deepEqual( getShape( out[ 0 ] ), [ 2, 2 ], 'returns expected value' );
	t.deepEqual( getShape( out[ 1 ] ), [ 1, 1 ], 'returns expected value' );
	t.deepEqual( getShape( out[ 2 ] ), [ 1, 1 ], 'returns expected value' );
	t.deepEqual( getShape( out[ 3 ] ), [ 1, 1 ], 'returns expected value' );
	t.deepEqual( getShape( out[ 4 ] ), [ 1, 1 ], 'returns expected value' );
	t.deepEqual( getShape( out[ 5 ] ), [ 1, 1 ], 'returns expected value' );
	t.deepEqual( getShape( out[ 6 ] ), [ 1, 1 ], 'returns expected value' );
	t.deepEqual( getStrides( out[ 1 ] ), [ 0, 0 ], 'returns expected value' );
...
tape( 'the function returns an ndarray unchanged if an ndarray has at least two dimensions', function test( t ) {
	var out;
	var x;
	var y;
	var z;
	...
	t.strictEqual( isArray( out ), true, 'returns expected value' );
	t.deepEqual( getShape( out[ 0 ] ), [ 3, 2 ], 'returns expected value' );
